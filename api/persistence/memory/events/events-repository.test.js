...
describe("an Event repository", () => {
  ...
  describe("#find", () => {
    ...
    it("should return only the events that match the filters", async () => {
      repository.store(
        buildEvent({ id: "fake1", shouldMatch: true, should: { match: true } })
      );
      repository.store(
        buildEvent({ id: "fake2", shouldMatch: false, should: { match: true } })
      );
      repository.store(
        buildEvent({ id: "fake1", shouldMatch: true, should: { match: false } })
      );
      repository.store(
        buildEvent({ id: "fake4", shouldMatch: true, should: { match: true } })
      );
      const results = await repository.find({
        filters: [
